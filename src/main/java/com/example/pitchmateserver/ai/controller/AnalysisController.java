...
public class AnalysisController {
    ...
    @Operation(summary = "AI 분석 요청", description = """
            영상에 대한 AI 분석을 요청합니다. 영상 업로드 시 자동으로 실행되므로 별도 호출이 필요 없습니다.
    ...
    @Operation(summary = "분석 상태 조회", description = "분석 ID로 현재 분석 상태만 조회합니다. status: PENDING / IN_PROGRESS / COMPLETED / FAILED")
    ...
    @Operation(summary = "분석 결과 조회", description = "분석 ID로 전체 분석 결과를 조회합니다. COMPLETED 상태일 때만 speechRateWpm, silenceRatio, fillerWordCount 등이 채워집니다.")
    ...
    @Operation(summary = "영상 기준 분석 결과 조회", description = "영상 ID로 해당 영상의 분석 결과를 조회합니다. analysisId를 모를 때 사용하세요.")
    ...
    @Operation(summary = "분석 결과 삭제", description = "분석 결과를 삭제합니다. 삭제 후 다시 분석 요청을 할 수 있습니다.")
